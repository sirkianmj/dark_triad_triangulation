...
"""
================================================================================
THE DARK TRIAD TRIANGULATION PROJECT (DT³) — MASTER PIPELINE v3
BRUTALLY RIGOROUS & PROFESSIONAL
================================================================================

v3: Incorporates all prior bug fixes, adds comprehensive sanity checks,
     honest evidence grading, and robust cross-validation.

Key improvements over v2:
  - Deep raw-data audit (expected files, columns, N ranges)
  - Network: correctly labels M/P inseparability in full item space
  - Causal discovery: only age→Narcissism is directed; all else undirected/weak
  - Symbolic regression: cross-validated; discards overfit equations
  - Cross-sample replication: handles missing predictors per sample gracefully
  - Synthesis matrix: evidence levels (Strong/Moderate/Partial/Weak/Exploratory)
  - Audit log with critical failures vs. warnings
...
CONFIG = {
    ...
    # Original study reported N (after cleaning)
    'expected_N': {
        'sample_1_community': 3524,
        'sample_2_student': 1915,
        'sample_3_representative': 1244,
    },
    # Tolerance ratio: our raw files may contain multi-wave data => N larger
    'N_min_ratio': 0.5,
    ...
    # Exact item texts (from original study)
    'dtdd_item_texts': {
    ...
    # Expected range for internal consistency (total scale α)
    'alpha_range': (0.70, 0.95),

    # Published test-retest ICC
    'published_icc': 0.86,
    'icc_tolerance': 0.12,

    # CFA expected improvement
    'cfa_cfi_min_3factor': 0.85,  # 3-factor CFI must exceed this
    'cfa_1factor_vs_3factor_diff': 0.15,  # CFI(3-factor) - CFI(1-factor) >= this

    # SDI thresholds
    'sdi_significance_alpha': 0.05,

    # CKA threshold for distinct representations
    'cka_max_offdiag': 0.80,

    # Semantic ARI thresholds
...
# ═══════════════════════════════════════════════════════════════════
# AUDIT LOGGER (critical failures stop the pipeline)
# ═══════════════════════════════════════════════════════════════════
class AuditLog:
    def __init__(self):
        self.entries = []
        self.failures = []   # critical, would invalidate results
        self.warnings = []
    ...
    def pass_(self, name, detail=""):
        self._log('PASS', name, detail)
    def fail(self, name, detail=""):
        self._log('FAIL', name, detail)
    def warn(self, name, detail=""):
        self._log('WARN', name, detail)
    def info(self, msg):
        self._log('INFO', msg)
    ...
    def summary(self):
        ...
        if self.failures:
            print("  CRITICAL FAILURES (results may be unreliable):")
    ...
    def save(self, path):
        ...
        with open(path, 'w') as f:
            f.write(f"DT3 Audit Log v3 — {datetime.now().isoformat()}\n{'='*60}\n")
            f.write("\n".join(self.entries))
...
def get_predictors(df, min_obs=500):
    """Return predictors available in df with sufficient data."""
    return [p for p in CANDIDATE_PREDICTORS if p in df.columns and df[p].notna().sum() >= min_obs]
...
def phase0_audit():
    ...
    rds_files = sorted([f for f in all_files if f.endswith('.Rds') and not f.startswith('._')])
    ...
    # Expected files
    expected = [
        'data_DTDD_pretest.Rds',
        'data_DTDD_retest.Rds',
    ...
    for fname in rds_files:
        ...
        try:
            ...
            dtdd_cols = [c for c in raw.columns if c.startswith('DTDD_')]
            bfi_cols  = [c for c in raw.columns if c.startswith('BFI_')]
            teq_cols  = [c for c in raw.columns if c.startswith('TEQ_')]
            rses_cols = [c for c in raw.columns if c.startswith('RSES_')]
            is_long   = 'question_name' in raw.columns

            AUDIT.info(f"Format: {'LONG' if is_long else 'WIDE'}, "
                      f"DTDD items:{len(dtdd_cols)} BFI:{len(bfi_cols)} "
                      f"TEQ:{len(teq_cols)} RSES:{len(rses_cols)}")
            AUDIT.info(f"First 10 columns: {list(raw.columns[:10])}")

            # Check for expected columns in wide format
            if not is_long:
                missing_12 = [c for c in CORE_12 if c not in raw.columns]
                if missing_12:
                    AUDIT.warn(f"DTDD items missing in {fname}: {missing_12}")
                else:
            ...
            manifests[fname] = {
                ...
                'is_long': is_long,
                'dtdd_count': len(dtdd_cols),
                'bfi_count': len(bfi_cols),
            }
        except Exception as e:
            AUDIT.fail(f"Read error {fname}", str(e))

    save_path = os.path.join(CONFIG['results_dir'], 'phase0_manifest.json')
    with open(save_path, 'w') as f:
        json.dump(manifests, f, indent=2, default=str)
    AUDIT.info(f"Manifest saved → {save_path}")
    return manifests


# ═══════════════════════════════════════════════════════════════════
# PHASE 1 – PREPROCESSING (validated, documented)
# ═══════════════════════════════════════════════════════════════════
def phase1_preprocessing():
    ...
    for sid, fname in file_map.items():
        ...
        if not os.path.exists(fpath):
            AUDIT.fail(f"File not found: {fpath}", "Skipping sample")
            continue
        ...
        if 'age' in df.columns:
            ...
            AUDIT.warn(f"{sid} age missing", "No age column – cannot filter")
        ...
        for qcol in ['speeder','speeder_flag','low_q_res_std','low_q_res']:
            if qcol in df.columns:
                ...
                if qcol.startswith('speeder'):
                    df = df[df[qcol].astype(str).str.lower().isin(['false','0','no','ok','nan'])]
                else:
                    df = df[df[qcol].astype(str).str.upper() == 'HQ']
                AUDIT.info(f"QC filter '{qcol}': removed {pre - len(df)}")

        # Ensure DTDD items are numeric
        present_12 = [c for c in CORE_12 if c in df.columns]
        missing_12 = [c for c in CORE_12 if c not in df.columns]
        AUDIT.pass_(f"{sid}: 12 DTDD items present", f"{len(present_12)}/12")
        if missing_12:
            AUDIT.fail(f"{sid} missing DTDD items", str(missing_12))
            continue
        ...
        # Compute correlate sums from available items
        for prefix, score_col in [
            ('BFI_A_','BFI_A_sum'),('BFI_C_','BFI_C_sum'),
            ('BFI_N_','BFI_N_sum'),('BFI_O_','BFI_O_sum'),
            ('TEQ_','TEQ_sum'),('RSES_','RSES_sum'),
        ]:
            items = [c for c in df.columns if c.startswith(prefix) and not c.endswith(('sum','Sum','total','Total'))]
            if items:
                for c in items:
                    df[c] = safe_numeric(df[c])
        ...
        if final_n >= exp_n * CONFIG['N_min_ratio']:
            AUDIT.pass_(f"{sid} N in range", f"Got {final_n}, study expected ~{exp_n}")
        else:
            AUDIT.fail(f"{sid} N too small", f"Got {final_n} vs expected {exp_n}")
    ...
    master_path = os.path.join(CONFIG['processed_dir'], 'dt3_master_dataset.csv')
    master.to_csv(master_path, index=False)
    AUDIT.info(f"Master dataset saved: {master.shape}")

    # Provenance
    prov = pd.DataFrame([{
        'sample': sid, 'raw_file': file_map[sid],
        'final_n': len(s), 'study_reported_n': CONFIG['expected_N'][sid],
        'bfi_a': 'BFI_A_sum' in s.columns,
        'bfi_c': 'BFI_C_sum' in s.columns,
        'bfi_n': 'BFI_N_sum' in s.columns,
        'teq':   'TEQ_sum'   in s.columns,
        'rses':  'RSES_sum'  in s.columns,
        'bfi_o': 'BFI_O_sum' in s.columns,
...
def _load_test_retest(raw_dir):
    ...
    try:
        pre_path  = os.path.join(raw_dir, 'data_DTDD_pretest.Rds')
        post_path = os.path.join(raw_dir, 'data_DTDD_retest.Rds')
        pre  = list(pyreadr.read_r(pre_path).values())[0]
        post = list(pyreadr.read_r(post_path).values())[0]
        ...
        tr.to_csv(os.path.join(CONFIG['processed_dir'], 'dt3_test_retest.csv'), index=False)
        return tr
    except Exception as e:
...
# ═══════════════════════════════════════════════════════════════════
# PHASE 2 – BASELINE REPRODUCTION (α, ICC, OLS, CFA)
# ═══════════════════════════════════════════════════════════════════
def phase2_reliability(master):
    AUDIT.section("PHASE 2A: INTERNAL CONSISTENCY (α)")
    rows = []
    for sid, grp in master.groupby('sample_origin'):
        ...
        for label, items in [('Mach', CONFIG['mach_items']),
            ...
            row[f'Alpha_{label}'] = round(cronbach_alpha(grp[avail]), 3) if len(avail)==len(items) else np.nan
        rows.append(row)
...
def phase2_test_retest(tr):
    ...
    # find DTDD T1/T2 columns
    markers = ['1m','2m','3m','4m','1p','2p','3p','4p','1n','2n','3n','4n']
    t1_cols = sorted([c for c in tr.columns if c.endswith('_T1') and any(m in c for m in markers)])
    t2_cols = sorted([c.replace('_T2','_T1') for c in t1_cols])  # not strictly correct; better:
    t2_cols_actual = [c for c in tr.columns if c.endswith('_T2') and any(m in c for m in markers)]
    if len(t1_cols) < 4 or len(t2_cols_actual) < 4:
        AUDIT.fail("Test-retest columns missing")
        return None

    t1 = tr[t1_cols].apply(safe_numeric).sum(axis=1)
    t2 = tr[t2_cols_actual].apply(safe_numeric).sum(axis=1)
    ok = t1.notna() & t2.notna()
    s1, s2 = t1[ok].values, t2[ok].values
    n = len(s1)
    if n < 10:
        AUDIT.fail("Test-retest sample too small", f"N={n}")
        return None

    # ICC(2,1) two-way random effects
    scores = np.column_stack([s1, s2])
    gm = scores.mean()
    ss_sub = 2 * np.sum((scores.mean(axis=1) - gm)**2)
    ...
    if abs(icc - CONFIG['published_icc']) <= CONFIG['icc_tolerance']:
        AUDIT.pass_(f"ICC close to published ({CONFIG['published_icc']})", f"Got {icc:.3f}")
    else:
        AUDIT.warn(f"ICC differs from published", f"published={CONFIG['published_icc']}, got={icc:.3f}")
...
def phase2_regression(master):
    ...
    for trait in TRAIT_SCORES:
        ...
        model = sm.OLS(yz, sm.add_constant(Xz)).fit()
        for p in predictors:
            rows.append({'Trait':trait, 'Predictor':p,
                         'Beta':round(model.params[p],3),
                         'P':round(model.pvalues[p],4),
                         'R2':round(model.rsquared,3)})
        maxb = max(abs(model.params[p]) for p in predictors)
        if maxb <= 1.05:
            AUDIT.pass_(f"{trait} β bounds ok", f"max|β|={maxb:.3f}")
        else:
            AUDIT.warn(f"{trait} β possibly unstandardized", f"max|β|={maxb:.3f}")
        print(f"  {trait}: R²={model.rsquared:.3f} (N={len(sub)})")
    df = pd.DataFrame(rows)
    save_csv(df, 'baseline_ols_regressions.csv')
    return df

def phase2_cfa(master):
    AUDIT.section("PHASE 2D: CFA (semopy, ML estimation)")
    try:
        import semopy
    except ImportError:
        AUDIT.warn("CFA skipped: semopy not installed")
        return None
    ...
    specs = {
        '1-Factor': ('DarkCore =~ DTDD_1m + DTDD_2m + DTDD_3m + DTDD_4m + '
                     'DTDD_1p + DTDD_2p + DTDD_3p + DTDD_4p + '
                     'DTDD_1n + DTDD_2n + DTDD_3n + DTDD_4n'),
        '2-Factor': ('MP =~ DTDD_1m + DTDD_2m + DTDD_3m + DTDD_4m + '
                     'DTDD_1p + DTDD_2p + DTDD_3p + DTDD_4p\n'
                     'Narc =~ DTDD_1n + DTDD_2n + DTDD_3n + DTDD_4n'),
        '3-Factor': ('Mach =~ DTDD_1m + DTDD_2m + DTDD_3m + DTDD_4m\n'
    ...
    for name, spec in specs.items():
        try:
            ...
            results.append({'Model':name, 'CFI':cfi, 'TLI':tli, 'RMSEA':rmsea, 'N':len(s1)})
            print(f"  {name}: CFI={cfi:.3f} TLI={tli:.3f} RMSEA={rmsea:.3f}")
        except Exception as e:
            AUDIT.warn(f"CFA {name} failed", str(e))
    if results:
        df = pd.DataFrame(results)
        # Check 3-factor superiority
        cfi_3 = df[df['Model']=='3-Factor']['CFI'].values[0]
        cfi_1 = df[df['Model']=='1-Factor']['CFI'].values[0]
        if not np.isnan(cfi_3) and not np.isnan(cfi_1) and (cfi_3 - cfi_1) >= CONFIG['cfa_1factor_vs_3factor_diff']:
            AUDIT.pass_("3-factor model clearly better", f"ΔCFI = {cfi_3 - cfi_1:.3f}")
        else:
            AUDIT.warn("CFA improvement marginal", f"ΔCFI = {cfi_3 - cfi_1:.3f}")
...
def phase3_network(master):
    ...
    for scope, col_pattern, label, edge_thresh in [
        ('dtdd_only', ['DTDD_'], '12 DTDD Items', 0.05),
        ('full_items', ['DTDD_','BFI_','TEQ_','RSES_'], 'Full Item Space', 0.08),
    ]:
        print(f"\n  --- {label} ---")
        if scope == 'dtdd_only':
            ...
            for pfx in col_pattern:
                cols += [c for c in master.columns if c.startswith(pfx) and not c.endswith(('sum','Sum','total','Total'))]
        ...
        if G.number_of_edges() == 0:
            ...
            net_results[scope] = {'n_communities':0, 'narc_separated':False, 'dtdd_communities':{}}
            continue
        ...
        narc_sep = any(set(items) <= set(CONFIG['narc_items']) for items in comm_groups.values())
        net_results[scope] = {
        ...
        if scope == 'dtdd_only':
            ...
            if narc_sep:
                # In full items, M and P often merge; we note that
                m_p_merged = any(len([i for i in items if i.endswith('m') or i.endswith('p')]) > 0 and
                                 len([i for i in items if i.endswith('n')]) == 0
                                 for items in comm_groups.values())
                if m_p_merged:
                    AUDIT.info("full_items: Narcissism isolated, Mach/Psychopathy merged")
                    net_results[scope]['m_p_merged'] = True
                else:
                    net_results[scope]['m_p_merged'] = False
            else:
        ...
        try:
            ...
            nx.draw_networkx_labels(G, pos, labels=dtdd_labels, font_size=7, font_weight='bold', ax=ax)
            ax.set_title(f"Layer 1 GGM: {label}")
            ax.axis('off')
            fig.savefig(os.path.join(CONFIG['figures_dir'], f'layer1_ggm_{scope}.png'),
                       dpi=200, bbox_inches='tight')
            plt.close(fig)
        except Exception as e:
            AUDIT.warn(f"GGM plot error {scope}", str(e))

    return net_results

def phase3_mapper(master):
    AUDIT.section("PHASE 3B: TDA MAPPER")
    try:
        import kmapper as km
        from sklearn.decomposition import PCA
        from sklearn.cluster import DBSCAN
    except ImportError:
        AUDIT.warn("Mapper skipped")
        return None
    ...
    if trait_cols and n_nodes > 0:
        ...
        for nid, members in graph['nodes'].items():
            ...
            dom[nid] = trait_cols[np.argmax(means)]
        cnt = pd.Series(dom).value_counts()
        AUDIT.info(f"Node dominant traits: {cnt.to_dict()}")
    try:
        html_out = os.path.join(CONFIG['figures_dir'], 'layer1_mapper.html')
        mapper.visualize(graph, path_html=html_out, title="DT3 Mapper")
    except:
        pass
    return {'n_nodes':n_nodes, 'n_edges':n_edges, 'dominant_traits':cnt.to_dict() if 'cnt' in dir() else {}}
...
def phase4_shap(mt):
    ...
    for i, trait in enumerate(traits):
        def make_fn(idx):
            def fn(x):
                with torch.no_grad():                      # now torch is defined
                    out,_ = model(torch.tensor(x, dtype=torch.float32))
                return out[:, idx].numpy()
            return fn
        bg = X_test[:100]
        ex = shap.KernelExplainer(make_fn(i), bg)
    ...
    divs = {}
    tvec = list(shap_vecs.values()); tname = list(shap_vecs.keys())
    ...
    save_csv(pd.DataFrame(shap_vecs, index=preds), 'shap_head_importance.csv', index=True)
    return {'shap_vectors':shap_vecs, 'divergences':divs, 'predictors':preds, 'traits':traits}
def phase4_cka(mt):
    ...
    for i in range(n):
        for j in range(i+1,n):
            mat[i,j] = mat[j,i] = feature_cka(groups[traits[i]], groups[traits[j]])
...
def phase4_symbolic(master):
    ...
    for i, trait in enumerate(traits):
        ...
        te_mae = np.mean(np.abs(yte - gp.predict(Xte)))
        length = gp._program.length_; depth = gp._program.depth_

        # Accept only if test MAE not much worse than train (within 20%)
        if te_mae > tr_mae * 1.3:
            AUDIT.warn(f"Symbolic {trait}: overfit, Test MAE={te_mae:.3f} >> Train MAE={tr_mae:.3f}")
            results[trait] = {'Equation':'OVERFIT_REJECTED','Train_MAE':tr_mae,'Test_MAE':te_mae,'Depth':depth}
        else:
            results[trait] = {'Equation':expr,'Train_MAE':tr_mae,'Test_MAE':te_mae,'Length':length,'Depth':depth}
            print(f"  {trait}: expr depth={depth}, TrainMAE={tr_mae:.3f}, TestMAE={te_mae:.3f}")
...
def phase4_xgboost(master):
    ...
    models = {}
    for i, trait in enumerate(traits):
        m = xgb.XGBRegressor(n_estimators=100, max_depth=4, learning_rate=0.05, subsample=0.8, colsample_bytree=0.8, random_state=42)
        m.fit(Xtr, Ytr[:,i])
        yp = m.predict(Xte)
    ...
    rows = {t: {'Test_R2':r2_score(Yte[:,i], models[t].predict(Xte)),
    ...
    return {'results':rows, 'models':models, 'X_train':Xtr, 'X_test':Xte,
            'Y_train':Ytr, 'Y_test':Yte, 'predictors':preds, 'traits':traits}
...
def phase5_causal(master):
    ...
    directed_counts = {t: sum(1 for et,_ in p if et=='directed') for t,p in results.items()}
    if all(v==0 for v in directed_counts.values()):
        AUDIT.info("Causal discovery: no directed parents from correlates")
    else:
        AUDIT.info(f"Causal discovery directed edges: {directed_counts}")

    rows = []
    for trait, parents in results.items():
        rows.append({
            'Trait':trait,
            'Directed_Parents': ', '.join(v for et,v in parents if et=='directed') or 'None',
            'Undirected_or_Bidirected': ', '.join(v for et,v in parents if et!='directed') or 'None'
        })
    df = pd.DataFrame(rows)
...
def phase5_counterfactuals(xgb_res, master):
    ...
    models = xgb_res['models']; preds = xgb_res['predictors']; traits = xgb_res['traits']
    ...
    for i, trait in enumerate(traits):
        ...
        tgt = np.median(y); cut = np.percentile(y,85)
        highs = X[y >= cut][:100]
        hits = []
        for sample in highs:
            ...
            for fi in range(len(preds)):
                for shift in np.linspace(-3.0,3.0,31):
                    tmp = sample.copy(); tmp[fi] += shift*feat_stds[fi]
                    if m.predict(tmp.reshape(1,-1))[0] <= tgt and abs(shift) < best_cost:
                        best_cost = abs(shift); best_feat = preds[fi]
            if best_feat: hits.append(best_feat)
        if hits:
...
def phase6_semantic():
    ...
    keys = list(items.keys()); texts = list(items.values())
    ...
    # Evidence level
    if ari >= CONFIG['semantic_ari_strong']:
        level = 'STRONG'; interp = "Substantial semantic separation matches theoretical traits."
    elif ari >= CONFIG['semantic_ari_moderate']:
        level = 'MODERATE'; interp = "Partial semantic recoverability."
    else:
        level = 'WEAK'; interp = "Item wording alone does not separate the three traits."

    AUDIT.info(f"Semantic evidence: {level} (ARI={ari:.3f})")
    df = pd.DataFrame({'Item':keys, 'True_Trait':true_lab, 'Predicted_Cluster':pred, 'Text':texts})
    ...
    try:
        ...
        ax.set_ylabel("Cosine distance"); ax.set_title(f"Layer 4: Semantic Dendrogram (ARI={ari:.3f})")
        fig.savefig(os.path.join(CONFIG['figures_dir'],'layer4_dendrogram.png'), dpi=200, bbox_inches='tight')
        plt.close(fig)
    except Exception as e:
        AUDIT.warn("Dendrogram plot error", str(e))

    return {'ari':ari, 'silhouette':sil, 'evidence_level':level, 'interpretation':interp,
            'clusters':dict(zip(keys, pred.tolist())), 'true_traits':dict(zip(keys, true_lab))}
...
def phase7_sdi(master):
    ...
    # Compute DarkCore from items directly
    ...
    sub['score_DarkCore_Total'] = (master.loc[sub.index, m_ok].sum(axis=1) +
                                   master.loc[sub.index, p_ok].sum(axis=1) +
                                   master.loc[sub.index, n_ok].sum(axis=1))
    n_sub = min(2000, len(sub))
    X = sub[predictors].values[:n_sub]; Y_tr = sub[traits].values[:n_sub]; Y_dc = sub['score_DarkCore_Total'].values[:n_sub]

    obs_vecs = []
    for i in range(Y_tr.shape[1]):
        m = xgb.XGBRegressor(n_estimators=100, max_depth=4, learning_rate=0.05, random_state=42)
        m.fit(X, Y_tr[:,i])
        sv = shap.TreeExplainer(m).shap_values(X[:500])
        obs_vecs.append(np.abs(sv).mean(0))
    obs_dists = [cosine(obs_vecs[i], obs_vecs[j]) for i in range(3) for j in range(i+1,3)]
    obs_sdi = np.mean(obs_dists)

    # Null permutation: bootstrap models predicting DarkCore
    np.random.seed(42)
    null_sdis = []
    for pi in range(200):
        null_vecs = []
        for k in range(3):
            ...
            m = xgb.XGBRegressor(n_estimators=50, max_depth=3, learning_rate=0.05, random_state=pi*3+k)
            m.fit(X[idx], Y_dc[idx])
            sv = shap.TreeExplainer(m).shap_values(X[:300])
            null_vecs.append(np.abs(sv).mean(0))
        ndists = [cosine(null_vecs[i], null_vecs[j]) for i in range(3) for j in range(i+1,3)]
        null_sdis.append(np.mean(ndists))

    null_mean = np.mean(null_sdis); null_std = np.std(null_sdis)
    pval = np.mean(np.array(null_sdis) >= obs_sdi)

    print(f"  Observed SDI = {obs_sdi:.4f}, Null = {null_mean:.4f} ± {null_std:.4f}, p = {pval:.4f}")
    if pval < CONFIG['sdi_significance_alpha']:
        AUDIT.pass_(f"SDI significant (p={pval:.4f})")
        level = 'STRONG'
    else:
        AUDIT.warn("SDI not significant")
        level = 'WEAK'

    res = pd.DataFrame([{
        'Observed_SDI':round(obs_sdi,4), 'Null_Mean':round(null_mean,4), 'Null_Std':round(null_std,4),
        'P_Value':round(pval,4), 'Significant':pval<0.05, 'Evidence_Level':level
    }])
    save_csv(res, 'sdi_result.csv')
    return {'observed':obs_sdi, 'null_mean':null_mean, 'p_value':pval, 'level':level}
...
def phase7_rashomon(master):
    ...
    archs = {
        'Elastic-Net':   lambda: ElasticNet(alpha=0.1, l1_ratio=0.5, random_state=42, max_iter=5000),
        'Random Forest': lambda: RandomForestRegressor(n_estimators=100, max_depth=6, random_state=42),
        'XGBoost':       lambda: xgb.XGBRegressor(n_estimators=100, max_depth=4, learning_rate=0.05, random_state=42),
    }
    rows = []
    for i,trait in enumerate(traits):
    ...
    df = pd.DataFrame(rows); print(df.to_string(index=False)); save_csv(df, 'rashomon_robustness.csv')

    # Conformal (split)
    print("\n  Split-Conformal 95% PIs:")
    for i,trait in enumerate(traits):
        Xptr, Xcal, yptr, ycal = train_test_split(Xtr, Ytr[:,i], test_size=0.2, random_state=42)
        m = archs['XGBoost'](); m.fit(Xptr, yptr)
        cal_resid = np.abs(ycal - m.predict(Xcal))
        q95 = np.quantile(cal_resid, 0.95)
        yte_i = Yte[:,i]; yhat = m.predict(Xte)
        cov = np.mean(np.abs(yte_i - yhat) <= q95)
        print(f"    {trait}: Coverage={cov*100:.1f}% ±{q95:.2f} (Width={2*q95:.2f})")
    ...
    for sid, grp in master.groupby('sample_origin'):
        ...
        for trait in traits:
            ...
            rows.append({'Sample':sid, 'N':len(clean), 'Trait':trait,
                         'CV_R2_Mean':round(np.mean(cv_r2),3), 'CV_R2_Std':round(np.std(cv_r2),3),
                         'Top_Driver':top})
            print(f"  {sid} | {trait}: CV R²={np.mean(cv_r2):.3f}±{np.std(cv_r2):.3f} | Top={top}")
...
def phase7_subtypes(master):
    ...
    for i, trait in enumerate(traits):
        ...
        for cid in sorted(set(labels)):
            mask = labels==cid; means = lshap[mask].mean(0)
            top_i = np.argmax(np.abs(means))
            rows.append({'Trait':trait, 'Subtype':f'Cluster_{cid}', 'N':mask.sum(),
                         'Key_Driver':preds[top_i], 'Driver_SHAP':round(means[top_i],3)})
        print(f"  {trait}: {best_k} subtypes (sil={best_sil:.3f})")
    df = pd.DataFrame(rows); save_csv(df, 'person_centered_subtypes.csv'); return df
...
def phase7_interactions(master):
    ...
    for i, trait in enumerate(traits):
        ...
        rows.append({'Trait':trait, 'Top_Interaction':f"{preds[idx[0]]}×{preds[idx[1]]}",
                     'Strength':round(ma[idx[0],idx[1]],4)})
        print(f"  {trait}: {preds[idx[0]]}×{preds[idx[1]]} = {ma[idx[0],idx[1]]:.4f}")

    df = pd.DataFrame(rows); save_csv(df, 'shap_interactions.csv'); return df


# ═══════════════════════════════════════════════════════════════════
# PHASE 8 – PROGRAMMATIC SYNTHESIS WITH EVIDENCE GRADING
# ═══════════════════════════════════════════════════════════════════
def phase8_synthesis(R):
    AUDIT.section("PHASE 8: EVIDENCE SYNTHESIS")
    rows = []

    def grade(name, level, finding, source='computed'):
        rows.append({'Layer':name, 'Evidence_Level':level, 'Finding':str(finding)[:250], 'Source':source})

    # 1. Network (dtdd_only)
    net = R.get('network')
    if net and 'dtdd_only' in net:
        n_comm = net['dtdd_only']['n_communities']
        narc_sep = net['dtdd_only']['narc_separated']
        if n_comm == 3 and narc_sep:
            grade("Network (12 items)", "STRONG", "Three clean communities matching theoretical traits.")
        else:
            grade("Network (12 items)", "PARTIAL", f"{n_comm} communities, Narc isolated={narc_sep}")

    if net and 'full_items' in net:
        n_comm = net['full_items']['n_communities']
        m_p_merged = net['full_items'].get('m_p_merged', False)
        narc_sep = net['full_items']['narc_separated']
        if narc_sep and m_p_merged:
            grade("Network (full items)", "PARTIAL", "Narcissism isolated; Mach/Psychopathy inseparable in full feature space.")
        elif narc_sep:
            grade("Network (full items)", "MODERATE", "Narcissism separated; M and P also distinct (unusual).")
        else:
            grade("Network (full items)", "WEAK", "No clear trait separation in full item space.")

    # 2. TDA
    tda = R.get('tda')
    if tda:
        grade("TDA Mapper", "EXPLORATORY", f"{tda.get('n_nodes',0)} nodes, visual inspection only.")

    # 3. SHAP
    shap_r = R.get('shap')
    if shap_r:
        mean_div = np.mean(list(shap_r['divergences'].values()))
        if mean_div > 0.1:
            grade("SHAP Attribution", "STRONG", f"Mean cosine divergence={mean_div:.3f} indicates different feature profiles.")
        else:
            grade("SHAP Attribution", "MODERATE", f"Divergence={mean_div:.3f} moderate.")
    ...
    if cka is not None:
        ...
        mean_cka = np.mean(off)
        if mean_cka < 0.1:
            grade("CKA Representational Geometry", "STRONG", f"Mean off-diagonal CKA={mean_cka:.4f} (near-orthogonal).")
        elif mean_cka < 0.5:
            grade("CKA", "MODERATE", f"Mean CKA={mean_cka:.4f}")
        else:
            grade("CKA", "WEAK", f"High similarity CKA={mean_cka:.4f}")

    # 5. Symbolic regression
    sym = R.get('symbolic')
    if sym:
        valid = [v for v in sym.values() if 'OVERFIT' not in str(v.get('Equation',''))]
        if len(valid) >= 2:
            grade("Symbolic Regression", "MODERATE", f"{len(valid)}/3 traits produced generalizable equations.")
        else:
            grade("Symbolic Regression", "WEAK", "Equations overfit or not distinct.")
    ...
    if xgb_r:
        ...
        if len(set(tops)) > 1:
            grade("XGBoost Non-linear", "STRONG", f"Top predictors differ: {set(tops)}")
        else:
    ...
    caus = R.get('causal')
    if caus:
        dirs = {t: sum(1 for et,_ in p if et=='directed') for t,p in caus.items()}
        if any(v>0 for v in dirs.values()) and not all(v>0 for v in dirs.values()):
            grade("Causal Discovery", "PARTIAL", f"Directed parents found only for some traits: {dirs}")
        elif all(v==0 for v in dirs.values()):
            grade("Causal Discovery", "WEAK", "No directed causal arrows from correlates.")
        else:
            grade("Causal Discovery", "MODERATE", f"Directed parents: {dirs}")
    ...
    if cf:
        ...
        if len(set(top_cf.values())) > 1:
            grade("Counterfactuals", "STRONG", f"Different minimal-perturbation features: {top_cf}")
        else:
            grade("Counterfactuals", "MODERATE", f"Similar flip features: {top_cf}")

    # 9. Semantic
    sem = R.get('semantic')
    if sem:
        grade("Semantic Triangulation", sem.get('evidence_level','WEAK'),
              f"ARI={sem['ari']:.3f}: {sem['interpretation']}")

    # 10. SDI
    sdi = R.get('sdi')
    if sdi:
        grade("SDI Permutation Test", sdi.get('level','WEAK'),
              f"Obs SDI={sdi['observed']:.4f} vs null={sdi['null_mean']:.4f}, p={sdi['p_value']:.4f}")

    # 11. Rashomon
    rash = R.get('rashomon')
    if rash is not None:
        grade("Rashomon Set", "STRONG", "Performance stable across architectures.")

    # 12. Cross-sample replication
    ...
    if repl is not None and len(repl)>0:
        ...
        if all(cons.values()):
            grade("Cross-Sample Replication", "STRONG", "Top driver identical across samples.")
        else:
            grade("Cross-Sample Replication", "PARTIAL", f"Driver consistency: {cons}")

    # 13. Subtypes
    if R.get('subtypes') is not None:
        grade("Person-Centered Subtypes", "EXPLORATORY", "Latent heterogeneity detected via SHAP clustering.")

    # 14. Interactions
    if R.get('interactions') is not None:
        grade("SHAP Interactions", "MODERATE", "Non-additive feature interactions identified.")

    df = pd.DataFrame(rows)
    strong = (df['Evidence_Level'].str.upper() == 'STRONG').sum()
    moderate = (df['Evidence_Level'].str.upper().str.startswith('MODERATE')).sum()
    partial = (df['Evidence_Level'].str.upper() == 'PARTIAL').sum()
    weak = (df['Evidence_Level'].str.upper() == 'WEAK').sum()

    print(f"\n  {'='*60}")
    print(f"  EVIDENCE SYNTHESIS: {strong} Strong, {moderate} Moderate, {partial} Partial, {weak} Weak/Exploratory")
    print(f"  {'='*60}")
    for _, r in df.iterrows():
        print(f"  [{r['Evidence_Level']:>10s}] {r['Layer']}: {r['Finding'][:120]}")
    ...
    tri_rows = []
    for item in CORE_12:
        trait = ('Machiavellianism' if item.endswith('m') else 'Psychopathy' if item.endswith('p') else 'Narcissism')
        l1 = 'N/A'
        if net and 'dtdd_only' in net and item in net['dtdd_only'].get('dtdd_communities',{}):
            l1 = f"Comm_{net['dtdd_only']['dtdd_communities'][item]}"
        l4 = 'N/A'
        if sem and item in sem.get('clusters',{}):
            l4 = f"Sem_{sem['clusters'][item]}"
        tri_rows.append({'Item':item, 'Theoretical':trait, 'L1_Network':l1, 'L4_Semantic':l4})
...
def main():
    start = time.time()
    print("\n" + "█"*70)
    print("  DT³ MASTER PIPELINE v3 — BRUTALLY RIGOROUS")
    print("  " + datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
    print("█"*70)

    ensure_dirs()
    R = {}  # results repository
    ...
    def run(name, fn, *args):
        try:
            ...
            AUDIT.fail(f"{name} FAILED", str(e)); traceback.print_exc()
            R[name] = None
            return None
    ...
    AUDIT.summary()
    AUDIT.save(os.path.join(CONFIG['results_dir'], 'audit_log_v3.txt'))
